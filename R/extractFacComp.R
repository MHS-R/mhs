...
GenData <- function(Supplied.Data, N.Factors, N, Max.Trials = 5, 
    ...
    while (Trials.Without.Improvement < Max.Trials) {
        ...
        # Calculate factor loadings and apply to reproduce desired
        # correlations (steps 7, 8) ---------------------------
        ...
        # Calculate RMSR correlation, compare to lowest value, take
        # appropriate action (steps 10, 11, 12) --------------
...
################################################################################################################ 
Factor.Analysis <- function(Data, Corr.Matrix = FALSE, Max.Iter = 50, 
    N.Factors = 0, Cor.Type) {
    Data <- as.matrix(Data)
    k <- dim(Data)[2]
    if (N.Factors == 0) {
        N.Factors <- k
