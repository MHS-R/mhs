...
likert.bar.plot2 <- function(l, low.color = "#D8B365", high.color = "#5AB4AC", 
    ...
    if (!is.null(colors) & length(colors) == l$nlevels) {
        ...
        bamp <- rgb(bamp(seq(0, 1, length = length(highrange) + 1)), 
            maxColorValue = 255)
        cols <- NULL
        if (center%%1 != 0) {
    ...
    if (!is.null(l$grouping)) {
        ...
        if (plot.percent.low) {
            p <- p + geom_text(data = lsum, y = ymin, aes(x = Group, 
                label = paste0(round(low), "%"), group = Item), size = text.size, 
        ...
        if (plot.percent.neutral & l$nlevels%%2 == 1 & include.center) {
            if (centered) {
                ...
                p <- p + geom_text(data = lsum, aes(x = Group, y = y, 
                  group = Item, label = paste0(round(neutral), "%")), 
                  size = text.size, hjust = 0.5, color = text.color)
            }
        }
        if (FALSE & plot.percents) {
            ...
            # label=paste0(round(value), '%'), group=Item), size=text.size)
            # lpercentneg <- results[results$value < 0,] if(nrow(lpercentneg) >
            # 0) { lpercentneg <- lpercentneg[nrow(lpercentneg):1,]
            # lpercentneg$value <- abs(lpercentneg$value) lpercentneg <-
            # ddply(lpercentneg, .(Item), transform, pos = cumsum(value) -
            # 0.5*value) lpercentneg$pos <- lpercentneg$pos * -1 p <- p +
        ...
        if (plot.percent.neutral & l$nlevels%%2 == 1 & include.center & 
            ...
            if (centered) {
                ...
                lsum$y <- lsum$low + (lsum$neutral/2)
                p <- p + geom_text(data = lsum, aes(x = Item, y = y, 
                  label = paste0(round(neutral), "%")), size = text.size, 
                  hjust = 0.5, color = text.color)
            }
        }
